 ...
 * @date Apr 9, 2013
 */
public class LIFOSchedulingAlgorithm extends BaseSchedulingAlgorithm {
    ...
    public void run() {    	
    	...
    	Stack<Cloudlet> tasks = new Stack<Cloudlet>();
        for (Iterator it = getCloudletList().iterator(); it.hasNext();) {
            tasks.push((Cloudlet)it.next());
        }
        while(!tasks.empty()) {
        	Cloudlet cloudlet = (Cloudlet) tasks.pop();
